...
* `gmx editconf -f atp_lyspro.gro -o atp30_newbox.gro -c -d 1.0 -bt cubic -box 6 6 6`
...
* `gmx genion -s ions.tpr -o 1iee_solv_ions.gro -p topol.top -pname SOD -np 120 -nname CLA -nn 13`

* `gmx grompp -f minim.mdp -c 1iee_solv_ions.gro -p topol.top -o em.tpr -maxwarn 1`

NOTE: SYSTEM HAS TOTAL CHARGE OF 0.2 

* `gmx mdrun -v -deffnm em`

* ` gmx energy -f em.edr -o potential.xvg` check if system has minimized energy.

* `gmx grompp -f nvt.mdp -c em.gro -r em.gro -p topol.top -o nvt.tpr -maxwarn 1`

* `gmx mdrun -v -deffnm nvt`

* `gmx energy -f nvt.edr -o temperature.xvg` check if system has equilibrated temperature

* `gmx grompp -f npt.mdp -c nvt.gro -r nvt.gro -t nvt.cpt -p topol.top -o npt.tpr -maxwarn 1`

* `gmx mdrun -v -deffnm npt`

`gmx energy -f npt.edr -o pressure.xvg` check if system has equilibrated pressure and density `gmx energy -f npt.edr -o density.xvg`
